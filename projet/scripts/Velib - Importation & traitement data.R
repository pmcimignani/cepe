...
Stations_velib <- read.csv("data/stations-velib-disponibilites-en-temps-reel.csv",sep=";")
...
datacomplete$day <- mday(datacomplete$download_date)
datacomplete$wday <- wday(datacomplete$download_date,label=T,abbr = F)
datacomplete$time <- format(as.POSIXct(datacomplete$download_date,format="%Y-%m-%d %H:%M:%S"),format="%H:%M")
datacomplete$bikes_availability <- datacomplete$available_bikes/datacomplete$bike_stands
datacomplete$stands_availability <- datacomplete$available_bike_stands/datacomplete$bike_stands